...
class LinearRequestFlowHardeningTest extends TestCase
{
    ...
    public function test_plan_change_removes_answers_that_no_longer_apply(): void
    {
        ...
        $request->respuestas()->create(['pregunta_id'=>$question32->id,'respuesta_texto'=>'Respuesta profesional','origen'=>'cliente']);
        ...
        $this->assertDatabaseMissing('solicitud_respuestas', ['solicitud_id'=>$request->id,'pregunta_id'=>$question32->id]);
    }
    ...
    public function test_registration_details_can_be_corrected_from_public_request(): void
    {
        [$request,$initial] = $this->publicRequestFixture();
        $user = Usuario::create([
            'cliente_id'=>$request->cliente_id,'nombre'=>'Nombre anterior','usuario'=>'cliente_lineal',
            'documento'=>'12345678','telefono'=>'70001111','password'=>'Prueba1234','rol'=>'cliente','estado'=>'activo',
        ]);
        ...
        $this->putJson('/api/v1/publico/solicitudes/'.$request->public_token, [
            ...
            'respuestas'=>[],
            'registro'=>[
                'cliente_nombre'=>'Nombre corregido','cliente_whatsapp'=>'71112222','cliente_ciudad'=>'Trinidad',
                'cliente_direccion'=>'Zona Central','empresa_nombre'=>'Empresa Corregida','empresa_actividad'=>'Servicios técnicos',
                'empresa_ciudad'=>'Trinidad','titulo_sistema'=>'Sistema técnico corregido','resumen'=>'Resumen corregido',
            ],
        ])->assertOk();

        $this->assertDatabaseHas('clientes', ['id'=>$request->cliente_id,'nombre'=>'Nombre corregido','whatsapp'=>'71112222','ciudad'=>'Trinidad']);
        ...
        $this->assertDatabaseHas('solicitudes_sistema', ['id'=>$request->id,'titulo'=>'Sistema técnico corregido','resumen'=>'Resumen corregido']);
    }
    ...
    public function test_new_request_automatically_uses_only_registered_company(): void
    {
        ...
        $company = Empresa::create([
            'cliente_id'=>$client->id,'codigo'=>'EMP-LINEAR-ONE','nombre_comercial'=>'Empresa única',
            'actividad'=>'Servicio técnico','estado'=>'activo',
        ]);

        $response = $this->actingAs($user)->postJson('/api/v1/mi/solicitud')
            ->assertCreated()->assertJsonPath('data.empresa.id',$company->id);

        $this->assertDatabaseHas('solicitudes_sistema', ['id'=>$response->json('data.id'),'empresa_id'=>$company->id,'cliente_id'=>$client->id]);
    }
    ...
    public function test_new_request_requires_company_choice_when_client_has_multiple_companies(): void
    {
        ...
        Empresa::create(['cliente_id'=>$client->id,'codigo'=>'EMP-LINEAR-A','nombre_comercial'=>'Empresa A','estado'=>'activo']);
        $companyB = Empresa::create(['cliente_id'=>$client->id,'codigo'=>'EMP-LINEAR-B','nombre_comercial'=>'Empresa B','estado'=>'activo']);
        ...
        $response = $this->actingAs($user)->postJson('/api/v1/mi/solicitud', ['empresa_id'=>$companyB->id])
            ->assertCreated()->assertJsonPath('data.empresa.id',$companyB->id);

        $this->assertDatabaseHas('solicitudes_sistema', ['id'=>$response->json('data.id'),'empresa_id'=>$companyB->id]);
    }
    ...
    private function publicRequestFixture(): array
    {
        ...
        $client = Cliente::create([
            'nombre'=>'Cliente lineal','telefono'=>'70001111','whatsapp'=>'70001111','ciudad'=>'Santa Cruz','estado'=>'formulario_en_proceso',
        ]);
        $company = Empresa::create([
            'cliente_id'=>$client->id,'codigo'=>'EMP-LINEAR-PUBLIC','nombre_comercial'=>'Empresa Lineal','actividad'=>'Servicios','estado'=>'pendiente_revision',
        ]);
        ...
        $request = SolicitudSistema::create([
            ...
            'codigo'=>'SOL-LINEAR-PUBLIC','public_token'=>Str::random(48),'publico_habilitado'=>true,
            'titulo'=>'Sistema Lineal','estado'=>'borrador','prioridad'=>'normal','acuerdo_comercial_requerido'=>true,
        ]);
    ...
    private function clientAccount(string $suffix): array
    {
        $client = Cliente::create([
            ...
            'ciudad'=>'Santa Cruz','estado'=>'activo',
        ]);
        $user = Usuario::create([
            'cliente_id'=>$client->id,'nombre'=>$client->nombre,'usuario'=>'client_'.strtolower($suffix),
            'documento'=>$client->documento,'telefono'=>$client->telefono,'password'=>'Prueba1234','rol'=>'cliente','estado'=>'activo',
        ]);
        return [$client,$user];
    }
}
