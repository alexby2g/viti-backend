...
return new class extends Migration {
    public function up(): void
    {
        ...
        DB::table('invitaciones_clientes')->whereNull('codigo')->orderBy('id')->eachById(function (object $invitation): void {
            do { $codigo='VITI-'.Str::upper(Str::random(6)); }
            while (DB::table('invitaciones_clientes')->where('codigo',$codigo)->exists());
            DB::table('invitaciones_clientes')->where('id',$invitation->id)->update(['codigo'=>$codigo,'updated_at'=>now()]);
        });
    }
