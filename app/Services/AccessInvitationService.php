...
class AccessInvitationService
{
    public function createAndSend(SolicitudSistema $solicitud, ?Usuario $actor, int $days = 7): array
    {
        $invitation = DB::transaction(function () use ($solicitud, $actor, $days): InvitacionCliente {
            $solicitud = SolicitudSistema::query()->with(['cliente.usuario','empresa'])->lockForUpdate()->findOrFail($solicitud->id);
            ...
            if ($email === '') {
                throw ValidationException::withMessages(['correo' => 'Registra el correo del responsable antes de enviar una invitación.']);
            }

            $active = InvitacionCliente::query()->where('solicitud_id', $solicitud->id)->whereIn('estado', ['pendiente','enviada'])->latest('id')->lockForUpdate()->first();
    ...
    {
        $invitation = InvitacionCliente::query()->where('solicitud_id', $solicitud->id)->whereIn('estado', ['pendiente','enviada'])->latest('id')->first();
        ...
        if ($currentEmail !== $invitation->correo_destino) $invitation->update(['correo_destino' => $currentEmail]);
    ...
    public function snapshotForSolicitud(SolicitudSistema $solicitud): array
    {
        ...
        $cliente = $solicitud->cliente;
        if (!$cliente) return ['estado'=>'sin_responsable','correo'=>null,'tiene_cuenta'=>false,'puede_enviar'=>false];
        if ($cliente->usuario) return ['estado'=>'cuenta_creada','correo'=>$cliente->correo ?: $cliente->usuario->correo,'tiene_cuenta'=>true,'usuario'=>$cliente->usuario->usuario,'puede_enviar'=>false];

        $invitation = InvitacionCliente::query()->where('solicitud_id', $solicitud->id)->latest('id')->first();
        if (!$invitation) return ['estado'=>filled($cliente->correo) ? 'pendiente_aprobacion' : 'sin_correo','correo'=>$cliente->correo,'tiene_cuenta'=>false,'puede_enviar'=>filled($cliente->correo)];
    ...
    public function snapshot(InvitacionCliente $invitation, ?bool $sentNow = null): array
    {
        ...
        return [
            'id'=>$invitation->id,'estado'=>$state,'correo'=>$invitation->correo_destino ?: $invitation->cliente?->correo,
            'tiene_cuenta'=>(bool) $invitation->cliente?->usuario,'expira_at'=>$invitation->expira_at,'enviada_at'=>$invitation->enviada_at,
            'ultimo_envio_at'=>$invitation->ultimo_envio_at,'intentos_envio'=>(int) $invitation->intentos_envio,'usada_at'=>$invitation->usada_at,
            'revocada_at'=>$invitation->revocada_at,'error_envio'=>$invitation->error_envio,'ruta'=>$active ? $route : null,
            'url'=>$active ? $front.$route : null,'email_enviado'=>$sentNow ?? (bool) $invitation->enviada_at,
            'puede_enviar'=>$active,'puede_reenviar'=>$active,'puede_revocar'=>$active,
        ];
    }
    ...
    {
        $provider = Str::lower(trim((string) config('services.transactional_mail.provider', 'brevo')));
        try {
            if ($provider === 'brevo') {
                ...
                if (in_array($transport, ['log','array'], true)) {
                    $invitation->update(['ultimo_envio_at'=>now(),'intentos_envio'=>((int)$invitation->intentos_envio)+1,'error_envio'=>'El servicio de correo todavía no está configurado para entrega real.']);
                    return false;
                }
                Mail::to($invitation->correo_destino)->send(new VitiAccessInvitation($invitation));
            }
            $invitation->update(['estado'=>'enviada','enviada_at'=>$invitation->enviada_at ?: now(),'ultimo_envio_at'=>now(),'intentos_envio'=>((int)$invitation->intentos_envio)+1,'error_envio'=>null]);
            return true;
        } catch (Throwable $e) {
            report($e);
            $invitation->update(['ultimo_envio_at'=>now(),'intentos_envio'=>((int)$invitation->intentos_envio)+1,'error_envio'=>Str::limit($e->getMessage(),1000)]);
            return false;
        }
    }
}
