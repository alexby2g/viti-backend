...
class FitFamilyAdminController extends Controller
{
    ...
    public function updateCategory(Request $request, FitFamilyCategoria $category): JsonResponse
    {
        ...
        if ($data['nombre'] !== $category->nombre) {
            $data['slug'] = $this->uniqueSlug(FitFamilyCategoria::class, $data['nombre'], $application->id, (int) $category->id);
        }
    ...
    public function updateProduct(Request $request, FitFamilyProducto $product): JsonResponse
    {
        ...
        if ($data['nombre'] !== $product->nombre) {
            $data['slug'] = $this->uniqueSlug(FitFamilyProducto::class, $data['nombre'], $application->id, (int) $product->id);
        }
        $product->update($data);
    ...
    private function assertCategoryBelongsToApplication(?int $categoryId, int $empresaId, int $applicationId): void
    {
        ...
        $valid = FitFamilyCategoria::query()
            ->whereKey($categoryId)
            ->where('empresa_id', $empresaId)
            ->where('aplicacion_id', $applicationId)
            ->exists();
    ...
    private function uniqueSlug(string $model, string $name, int $applicationId, ?int $ignoreId = null): string
    {
        ...
        while ($model::query()->where('aplicacion_id', $applicationId)->when($ignoreId, fn ($q) => $q->whereKeyNot($ignoreId))->where('slug', $slug)->exists()) {
            $slug = $base . '-' . $counter++;
        }
