...
class SolicitudAccesoVitiController extends Controller
{
    public function store(Request $request): JsonResponse
    {
        ...
        abort_if($recent,422,'Ya existe una solicitud de acceso reciente para este teléfono. VITI la revisará antes de crear un nuevo acceso.');

        $access = SolicitudAccesoViti::create([...$data,'estado'=>'pendiente']);

        return response()->json([
            'message'=>'Solicitud recibida. VITI revisará tus datos y, si corresponde, te enviará un acceso personal.',
            'data'=>['id'=>$access->id,'estado'=>$access->estado,'plan_codigo'=>$access->plan_codigo,'modalidad'=>$access->modalidad],
        ],201);
    }
    ...
    public function approve(Request $request, SolicitudAccesoViti $solicitud): JsonResponse
    {
        ...
        [$access,$invitation]=DB::transaction(function() use($request,$solicitud,$data): array {
            ...
            $invitation=InvitacionCliente::create([
                'token'=>Str::random(64),'codigo'=>$this->newCode(),'creada_por'=>$request->user()->id,
                'estado'=>'pendiente','expira_at'=>now()->addDays($days),
            ]);
        ...
        return response()->json(['message'=>'Solicitud aprobada. Comparte el código y el enlace personal con el solicitante.','data'=>['solicitud'=>$access,'codigo'=>$invitation->codigo,'expira_at'=>$invitation->expira_at,'ruta'=>'/registro-cliente/'.$invitation->token]]);
    }
    ...
    {
        do {$code='VITI-'.Str::upper(Str::random(6));} while (InvitacionCliente::where('codigo',$code)->exists());
        return $code;
    }
}
