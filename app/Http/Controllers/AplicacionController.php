...
{
 public function index(Request $r):JsonResponse{$q=Aplicacion::with(['empresa:id,nombre_comercial','proyecto:id,nombre,codigo'])->latest();if($r->filled('buscar')){$t='%'.$r->string('buscar').'%';$q->where(fn($x)=>$x->where('nombre','like',$t)->orWhereHas('empresa',fn($e)=>$e->where('nombre_comercial','like',$t)));}return response()->json($q->paginate(20));}
 ...
 public function show(Aplicacion $aplicacion):JsonResponse{return response()->json(['data'=>$aplicacion->load(['empresa','proyecto','mantenimientos','archivos'])]);}
