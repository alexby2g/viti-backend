...
<div style="margin-bottom:9px"><div class="question">{{ $pregunta->numero }}. {{ $pregunta->pregunta }}</div><div class="answer">@if($answer?->respuesta_json){{ implode(', ', $answer->respuesta_json) }}@else{{ $answer?->respuesta_texto }}@endif</div></div>
@endforeach</div>
@endif
@endforeach
...
<p><strong>Precio referencial:</strong> {{ $solicitud->planViti?->precio_proyecto !== null ? number_format((float)$solicitud->planViti->precio_proyecto,2).' Bs' : 'Cotización personalizada' }}</p>
...
@endif
<div class="section"><h2>Declaración final del solicitante</h2><p>Confirmo que las respuestas proporcionadas representan de manera general la idea y las necesidades iniciales del sistema solicitado.</p><p><strong>Nombre:</strong> {{ $solicitud->declaracion_nombre ?: $solicitud->cliente?->nombre }}</p><p><strong>Fecha:</strong> {{ $solicitud->declaracion_fecha?->format('d/m/Y') ?: 'Sin confirmar' }} &nbsp;&nbsp; <strong>Declaración:</strong> {{ $solicitud->declaracion_aceptada ? 'Aceptada' : 'Pendiente' }}</p></div>
