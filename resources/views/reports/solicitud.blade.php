...
<div style="margin-bottom:9px"><div class="question">{{ $pregunta->numero }}. {{ $pregunta->pregunta }}</div>@if(($answer?->origen ?? 'cliente')==='tecnico')<div class="muted" style="font-size:10px;margin-bottom:2px">Definición técnica AGR Studio · no reemplaza la respuesta original de la empresa</div>@endif<div class="answer">@if($answer?->respuesta_json){{ implode(', ', $answer->respuesta_json) }}@else{{ $answer?->respuesta_texto }}@endif</div></div>
@endforeach</div>
@endif
@endforeach
...
<p>Esta selección expresa la preferencia inicial de la empresa. AGR Studio revisa que el plan cubra el alcance antes de aprobar el proyecto. Los desarrollos e integraciones fuera del plan se cotizan por separado.</p>
...
@if($solicitud->planViti?->precio_mensual !== null || $solicitud->planViti?->precio_anual !== null)
...
@endif
<div class="section"><h2>Declaración final</h2><p>Confirmo que las respuestas proporcionadas por la empresa representan de manera general la idea y las necesidades iniciales del sistema solicitado. Las definiciones técnicas agregadas posteriormente por AGR Studio se identifican expresamente como tales.</p><p><strong>Nombre:</strong> {{ $solicitud->declaracion_nombre ?: $solicitud->cliente?->nombre }}</p><p><strong>Fecha:</strong> {{ $solicitud->declaracion_fecha?->format('d/m/Y') ?: 'Sin confirmar' }} &nbsp;&nbsp; <strong>Declaración:</strong> {{ $solicitud->declaracion_aceptada ? 'Aceptada' : 'Pendiente' }}</p></div>
<div class="footer">VITI · Solicitud, diagnóstico y acuerdo comercial inicial</div></body></html>